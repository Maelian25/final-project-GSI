...
export const ChemicalImage: React.FC<ChemicalImageProps> = ({ casNumber }) => {
  const [imageUrl, setImageUrl] = useState("");

  useEffect(() => {
    const fetchChemicalImage = async () => {
      const cid = await convertCasToCid(casNumber);
      if (cid) {
        setImageUrl(
  ...
  return (
    <img
      src={imageUrl}
      alt={`Chemical structure for CAS Number: ${casNumber}`}
