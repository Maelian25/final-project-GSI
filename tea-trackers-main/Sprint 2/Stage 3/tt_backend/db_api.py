...
class ArticleUser(Base):
    ...
    def __repr__(self):
        return f"<ArticleUser(id_product={self.id_product})>"
...
class DatabaseAPI:
    ...
    def get_article_user(self, id_product: int, username: str):
        session = self.Session()
        article_user = session.query(ArticleUser).filter_by(id_product=id_product, username=username).first()
        session.close()
        return article_user

    def get_articles_user(self, username: str):
        session = self.Session()
    ...
    def get_all_articles_user(self):
        session = self.Session()
        articles_user = session.query(ArticleUser).all()
        session.close()
        return articles_user
    ...
    def delete_article_user(self, id_product: int, username: str):
        session = self.Session()
        article_user = session.query(ArticleUser).filter_by(id_product=id_product, id_user=username).first()
        session.delete(article_user)
        session.commit()
        session.close()

    def delete_all_articles_user(self, username: int):
        session = self.Session()
        articles_user = session.query(ArticleUser).filter_by(username=username).all()
    ...
    def search_produits(self, query: str):
        session = self.Session()
        produits = (
            session.query(Produit)
            .filter(
                Produit.name.ilike(f"%{query}%") |
                Produit.fabricant.ilike(f"%{query}%") |
                Produit.cas_number.ilike(f"%{query}%")
